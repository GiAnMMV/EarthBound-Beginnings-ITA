...
pointer_pos = [
    [0x01810,0x01dd0,0xb8,0x00,0x08],
    [0x01e10,0x02000,0x3e,0x00,0x08],
    [0x2c010,0x2cf50,0x7a,0x18,0x20],
    [0x3c62e,0x3c63e,0x08,0x00,0x02]
]
pointers = []
for a in pointer_pos:
    for i in range(a[2]):
        pointers.append(a[0]+i*a[4]+a[3])

strings_start = 0x10
strings_end = 0xc10

text_strings = deconvert(''.join(strings))
if len(strings) <= strings_end:
    rom = rom[:strings_start] + text_strings + b'\x00'*(strings_end-strings_start-len(text_strings)) + rom[strings_end:]
